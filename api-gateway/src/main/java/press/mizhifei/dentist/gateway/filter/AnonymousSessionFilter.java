...
/**
 * Filter for managing anonymous user sessions and header propagation
...
public class AnonymousSessionFilter implements GlobalFilter, Ordered {
    ...
    // Public endpoints that should receive anonymous session tracking
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // Get existing anonymous ID from request headers
        String existingAnonId = request.getHeaders().getFirst("ANON_ID");
        ...
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(authentication -> {
                    if (authentication != null && authentication.isAuthenticated()) {
                        ...
                        return handleAuthenticatedUser(exchange, chain, existingAnonId, authentication);
                    } else {
                        // Handle anonymous user
                        return handleAnonymousUser(exchange, chain, existingAnonId);
                    }
                })
    ...
    private Mono<Void> handleAuthenticatedUser(ServerWebExchange exchange, GatewayFilterChain chain,
                                             String existingAnonId, Authentication authentication) {
        ServerHttpRequest request = exchange.getRequest();
        ...
        if (StringUtils.hasText(authHeader) && authHeader.startsWith("Bearer ")) {
            ...
            AnonymousSessionService.SessionInfo sessionInfo = 
                    anonymousSessionService.linkToAuthenticatedUser(
                            existingAnonId, userId, email, roles, clinicId);
            ...
            log.debug("Authenticated user session - AnonID: {}, SessionID: {}, UserID: {}", 
                    sessionInfo.getAnonymousId(), sessionInfo.getSessionId(), userId);
    ...
    private Mono<Void> handleAnonymousUser(ServerWebExchange exchange, GatewayFilterChain chain,
                                         String existingAnonId) {
        // Get or create anonymous session
        AnonymousSessionService.SessionInfo sessionInfo = 
                anonymousSessionService.getOrCreateSession(existingAnonId);
        ...
        log.debug("Anonymous user session - AnonID: {}, SessionID: {}", 
                sessionInfo.getAnonymousId(), sessionInfo.getSessionId());
