...
SERVICES=(config-server discovery-server api-gateway auth-service audit-service system-service genai-service clinic-service appointment-service clinical-records-service user-profile-service admin-server notification-service chat-log-service)
...
if [[ "$BUILD_MODE" == "parallel" ]]; then
  ...
  start_builds auth-service audit-service system-service genai-service clinic-service appointment-service clinical-records-service user-profile-service admin-server notification-service chat-log-service
  wait_for_builds auth-service audit-service system-service genai-service clinic-service appointment-service clinical-records-service user-profile-service admin-server notification-service chat-log-service || exit 1
else
